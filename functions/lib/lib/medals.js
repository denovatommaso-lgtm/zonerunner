...
exports.medalId = medalId;
exports.medalFromId = medalFromId;
exports.medalsFromChallenges = medalsFromChallenges;
exports.medalsFromMonthlyHistory = medalsFromMonthlyHistory;
exports.medalsFromYearlyHistory = medalsFromYearlyHistory;
exports.medalTierCountsFromHistory = medalTierCountsFromHistory;
const monthlyChallengesConfig_1 = require("./monthlyChallengesConfig");
const monthlyChallengesConfig_2 = require("./monthlyChallengesConfig");
const monthlyChallenges_1 = require("./monthlyChallenges");
const allDefs = {
    monthly: monthlyChallengesConfig_1.monthlyChallenges,
    yearly: monthlyChallengesConfig_2.yearlyChallengeDefinitions,
};
// Base placeholders (guaranteed to exist).
const tierFallbackImages = {
...
function medalId(source, challengeId, stage) {
    return `medal:${source}:${challengeId}:${stage}`;
}
function medalFromId(id) {
    const parts = id.split(':');
    if (parts.length !== 4 || parts[0] !== 'medal')
        return null;
    const [, sourceRaw, challengeId, stageRaw] = parts;
    if (sourceRaw !== 'monthly' && sourceRaw !== 'yearly')
        return null;
    const source = sourceRaw;
    const defs = allDefs[source];
    const def = defs.find((d) => d.id === challengeId);
    const stage = Number(stageRaw);
    if (!def || !Number.isFinite(stage) || stage < 1)
        return null;
    const tier = stage >= 3 ? 'gold' : stage === 2 ? 'silver' : 'bronze';
    return {
        id,
        label: `${def.baseLabel} ${(0, monthlyChallenges_1.romanNumeral)(stage)}`,
        tier,
        source,
        image: medalImages[id] ?? tierFallbackImages[tier] ?? legacyFallback,
    };
}
function medalsFromChallenges(views, source) {
    return views
...
function completedStages(stage, starsEarned, maxStage) {
...
function rankingCompletedFromMeta(meta) {
...
function medalsFromYearlyHistory(state) {
    const awarded = new Set();
    const counted = new Set();
    for (const year of Object.values(state.years ?? {})) {
        for (const [challengeId, prog] of Object.entries(year.challenges ?? {})) {
            const def = monthlyChallengesConfig_2.yearlyChallengeDefinitions.find((d) => d.id === challengeId);
            if (!def)
                continue;
...
function incrementTier(counts, tier, inc = 1) {
    counts[tier] = (counts[tier] ?? 0) + inc;
}
function ensureCounts(map, challengeId) {
    if (!map[challengeId]) {
        map[challengeId] = { bronze: 0, silver: 0, gold: 0 };
    }
    return map[challengeId];
}
function medalTierCountsFromHistory(monthlyState, yearlyState) {
    const counts = {};
    const countedChallenges = new Set();
    ...
    for (const [challengeId, stage] of Object.entries(monthlyState.stageUnlocked ?? {})) {
        if (countedChallenges.has(challengeId))
            continue;
        const stageValue = typeof stage === 'number' ? stage : Number(stage) || 0;
    ...
    for (const [challengeId, stage] of Object.entries(yearlyState.stageUnlocked ?? {})) {
        if (countedChallenges.has(challengeId))
            continue;
        const stageValue = typeof stage === 'number' ? stage : Number(stage) || 0;
        const capped = Math.max(0, Math.min(stageValue, 3));
