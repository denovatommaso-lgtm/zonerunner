...
exports.computeFastestSegmentSeconds = computeFastestSegmentSeconds;
const geoMetrics_1 = require("./geoMetrics");
/**
 ...
 */
function computeFastestSegmentSeconds(points, opts) {
    if (!points || points.length < 3)
        return null;
    if (!Number.isFinite(opts.segmentMeters) || opts.segmentMeters <= 0)
        return null;
    if (!Number.isFinite(opts.minRunDistanceMeters) || opts.minRunDistanceMeters <= 0)
        return null;
    // Require timestamps for pace calculation.
    if (points.some((p) => typeof p.ts !== 'number'))
        return null;
    const n = points.length;
    const t = points.map((p) => p.ts);
    for (let i = 1; i < n; i++) {
    ...
    for (let i = 1; i < n; i++) {
        const d = (0, geoMetrics_1.haversineMeters)(points[i - 1], points[i]);
        cum[i] = cum[i - 1] + (Number.isFinite(d) ? Math.max(0, d) : 0);
    }
    const polylineTotal = cum[n - 1];
    const runDistanceMeters = typeof opts.runDistanceMeters === 'number' && Number.isFinite(opts.runDistanceMeters)
    ...
    const qualifyingTotal = typeof runDistanceMeters === 'number' && runDistanceMeters > 0 ? runDistanceMeters : polylineTotal;
    if (qualifyingTotal < opts.minRunDistanceMeters)
        return null;
    ...
    const factor = opts.allowScalePolylineToRunDistance &&
