...
exports.monthKeyFromEpochMsLocal = monthKeyFromEpochMsLocal;
exports.monthKeyFromLocalDate = monthKeyFromLocalDate;
exports.romanNumeral = romanNumeral;
exports.computeStarThresholds = computeStarThresholds;
exports.starsForProgress = starsForProgress;
exports.milestoneId = milestoneId;
exports.xpForMilestone = xpForMilestone;
exports.stageTotalXp = stageTotalXp;
exports.territoryRewardForMilestone = territoryRewardForMilestone;
exports.getChallengeDefinition = getChallengeDefinition;
exports.thresholdsForStage = thresholdsForStage;
exports.initMonthlyChallengesState = initMonthlyChallengesState;
exports.migrateMonthlyChallengesState = migrateMonthlyChallengesState;
exports.ensureMonthBucket = ensureMonthBucket;
exports.pruneHistory = pruneHistory;
exports.ensureMonthRollover = ensureMonthRollover;
exports.applyRunToMonthlyChallenges = applyRunToMonthlyChallenges;
exports.applyMonthlyEventIncrement = applyMonthlyEventIncrement;
exports.evaluateRankingStars = evaluateRankingStars;
exports.applyRankingSnapshotToMonthlyChallenges = applyRankingSnapshotToMonthlyChallenges;
exports.buildMonthlyChallengeViews = buildMonthlyChallengeViews;
const pace_1 = require("./geo/pace");
const monthlyChallengesConfig_1 = require("./monthlyChallengesConfig");
function bucketKeyForChallenge(challengeId, d) {
    // Countries/states are yearly; everything else monthly.
    const cadence = challengeId === 'countries' || challengeId === 'states' ? 'yearly' : 'monthly';
    if (cadence === 'yearly') {
        return `YEAR-${d.getFullYear()}`;
    }
    return monthKeyFromLocalDate(d);
}
function monthKeyFromEpochMsLocal(ts) {
    return monthKeyFromLocalDate(new Date(ts));
}
function pad2(n) {
    return n.toString().padStart(2, '0');
}
function monthKeyFromLocalDate(d) {
    return `${d.getFullYear()}-${pad2(d.getMonth() + 1)}`;
}
function monthStartEpochMsLocal(monthKey) {
...
function roundByUnit(unit, value) {
    const v = Math.max(0, value);
    if (unit === 'km')
        return Math.max(1, Math.round(v));
    if (unit === 'sec')
        return Math.max(300, Math.round(v / 300) * 300); // nearest 5 minutes
    if (unit === 'paceSec')
        return Math.max(5, Math.round(v / 5) * 5); // nearest 5 seconds
    return v;
}
function normalizeTimedRoute(run) {
    ...
    const hasMonotonic = valid.every((p) => typeof p.ts === 'number') &&
        valid.every((p, i) => (i === 0 ? true : p.ts > valid[i - 1].ts));
    if (hasMonotonic)
        return valid;
    const startMs = (() => {
...
function isQualifyingRunForCountChallenges(run) {
    const distOk = (run.distance ?? 0) >= monthlyChallengesConfig_1.monthlyRunQualification.minDistanceMeters;
    const timeOk = (run.elapsedSeconds ?? 0) >= monthlyChallengesConfig_1.monthlyRunQualification.minMovingSeconds;
    const qualifyMode = monthlyChallengesConfig_1.monthlyRunQualification.qualifyMode;
    return qualifyMode === 'and' ? distOk && timeOk : distOk || timeOk;
}
function minuteOfDayLocal(d) {
...
function computeStarThresholds(unit, target3Star) {
    if (unit === 'paceSec') {
        ...
        const round5s = (s) => Math.max(5, Math.round(s / 5) * 5);
...
function starsForProgress(progressValue, thresholds) {
...
function stageMilestoneXp(challengeId, stage) {
    ...
    const raw1 = total * (monthlyChallengesConfig_1.monthlyXpConfig.starSplit[1] ?? 0);
    const raw2 = total * (monthlyChallengesConfig_1.monthlyXpConfig.starSplit[2] ?? 0);
    let x1 = roundToNearest2(raw1);
    let x2 = roundToNearest2(raw2);
    // Keep within total.
    if (x1 + x2 > total) {
        // Reduce x2 then x1 in even steps until it fits.
        while (x1 + x2 > total && x2 >= 2)
            x2 -= 2;
        while (x1 + x2 > total && x1 >= 2)
            x1 -= 2;
    }
    let x3 = total - x1 - x2;
...
function xpForMilestone(challengeId, stage, star) {
    const split = stageMilestoneXp(challengeId, stage);
    return split[star] ?? 0;
}
function stageTotalXp(challengeId, stage) {
    const stageArr = monthlyChallengesConfig_1.monthlyXpConfig.stageTotalXpByChallenge[challengeId] || [];
    const raw = Math.max(0, stageArr[stage] ?? 0);
    // Keep totals even so per-star XP can be even too.
    return raw - (raw % 2);
}
function territoryRewardForMilestone(challengeId, stage, star) {
    const base = monthlyChallengesConfig_1.monthlyTerritoryRewardConfig.baseRewardByChallenge[challengeId] ?? 0;
    const stageMult = monthlyChallengesConfig_1.monthlyTerritoryRewardConfig.stageMultiplier[stage] ?? 1;
    return Math.max(0, Math.round(base * stageMult * star));
}
function getChallengeDefinition(id) {
    const found = monthlyChallengesConfig_1.monthlyChallenges.find((c) => c.id === id);
    if (!found) {
        // Should never happen; fallback to distance so UI doesn't crash.
        return monthlyChallengesConfig_1.monthlyChallenges[0];
    }
    return found;
}
function thresholdsForStage(def, stage) {
    const override = def.stageStarThresholds?.[stage - 1];
    if (override)
        return override;
    const target3 = def.stageTargets3Star[stage - 1] ??
        def.stageTargets3Star[def.stageTargets3Star.length - 1] ??
        1;
    return computeStarThresholds(def.unit, target3);
}
function initMonthlyChallengesState(now = new Date()) {
...
function migrateMonthlyChallengesState(state) {
    if ((state.version ?? 0) === monthlyChallengesConfig_1.monthlyChallengeSettings.version)
        return state;
    ...
    for (const [mk, month] of Object.entries(state.months ?? {})) {
        ...
        for (const [cid, prog] of Object.entries(month.challenges ?? {})) {
            ...
            if (mapped === 'ranking')
                continue;
            const p = prog;
            const rawMilestones = (p.milestonesGranted ?? []);
        ...
        months[mk] = {
            ...
            appliedRunIds: (month.appliedRunIds ?? []),
    ...
    for (const [mk, month] of Object.entries(months)) {
        for (const [challengeId, prog] of Object.entries(month.challenges ?? {})) {
            ...
            if (!mapped)
                continue;
            for (const mid of prog.milestonesGranted ?? []) {
                if (typeof mid !== 'string')
                    continue;
                if (!mid.startsWith(`${mk}:`))
                    continue;
                const stageMatch = mid.match(/:stage(\d+):star([123])$/);
                if (!stageMatch)
                    continue;
                const stage = Number(stageMatch[1]);
                const star = Number(stageMatch[2]);
...
function pruneHistory(state) {
    ...
    const max = monthlyChallengesConfig_1.monthlyChallengeSettings.maxHistoryMonths;
    if (keys.length <= max)
        return state;
...
function ensureMonthRollover(state, now = new Date()) {
    state = migrateMonthlyChallengesState(state);
    const mk = monthKeyFromLocalDate(now);
    if (state.lastMonthKey === mk)
        return state;
    let next = {
        ...state,
        lastMonthKey: mk,
...
function contributionForChallenge(ch, run) {
    if (ch === 'distance')
        return (run.distance ?? 0) / 1000;
    if (ch === 'time')
    ...
    if (ch === 'countries' || ch === 'states')
        return 0;
    // Longest run is best-of-month based; handled in applyRunToMonthlyChallenges.
    if (ch === 'longest')
        return 0;
    if (ch === 'earlyBird') {
        if (!isQualifyingRunForCountChallenges(run))
            return 0;
        const d = new Date(run.startedAt);
        const m = minuteOfDayLocal(d);
        return isWithinWindow(m, monthlyChallengesConfig_1.monthlyTimeWindows.earlyBird) ? 1 : 0;
    }
    if (ch === 'nightOwl') {
        if (!isQualifyingRunForCountChallenges(run))
            return 0;
        const d = new Date(run.startedAt);
        const m = minuteOfDayLocal(d);
        return isWithinWindow(m, monthlyChallengesConfig_1.monthlyTimeWindows.nightOwl) ? 1 : 0;
    }
    return 0;
}
function applyRunToMonthlyChallenges(params) {
    const { state, runId, run } = params;
    if (!run?.startedAt) {
        if (__DEV__)
            console.log('[MonthlyChallenges] ingest skipped missing startedAt', runId);
    ...
    if (month.appliedRunIds.includes(runId)) {
        ...
        const paceDef = monthlyChallengesConfig_1.monthlyChallenges.find((c) => c.id === 'pace');
        ...
        const pacePrevStars = paceThresholds && paceExisting ? paceExisting.starsEarned ?? starsForProgress(pacePrevProgress, paceThresholds) : 0;
        if (paceDef && paceThresholds && (pacePrevProgress ?? 0) <= 0) {
            const awarded = [];
            let xpDelta = 0;
            let territoryDelta = 0;
            const timedRoute = normalizeTimedRoute(run);
            let best = (0, pace_1.computeFastestSegmentSeconds)(timedRoute, {
                segmentMeters: monthlyChallengesConfig_1.monthlyPaceConfig.segmentMeters,
                minRunDistanceMeters: monthlyChallengesConfig_1.monthlyPaceConfig.minRunDistanceMeters,
                allowScalePolylineToRunDistance: monthlyChallengesConfig_1.monthlyPaceConfig.allowScalePolylineToRunDistance,
                runDistanceMeters: run.distance ?? 0,
            });
            if (!best &&
                typeof run.distance === 'number' &&
                run.distance >= monthlyChallengesConfig_1.monthlyPaceConfig.segmentMeters &&
                typeof run.elapsedSeconds === 'number' &&
                run.elapsedSeconds > 0) {
                const avgSecPerMeter = run.elapsedSeconds / Math.max(1, run.distance);
                best = Math.max(1, Math.round(avgSecPerMeter * monthlyChallengesConfig_1.monthlyPaceConfig.segmentMeters));
            }
            if (best) {
    ...
    let xpDelta = 0;
    let territoryDelta = 0;
    for (const def of monthlyChallengesConfig_1.monthlyChallenges) {
...
function applyMonthlyEventIncrement(params) {
...
function evaluateRankingStars(params) {
...
function applyRankingSnapshotToMonthlyChallenges(params) {
    ...
    const scopeStage = { state: 1, country: 2, world: 3 };
    ...
    const milestonesGranted = new Set(existing?.milestonesGranted ?? []);
    const meta = stripUndefined({ ...existing?.meta });
    ...
    const prevStars = prevScopeMeta.stars ?? { top25: false, top10: false, top3: false };
    const nextStars = {
    ...
    const bestRank = typeof prevScopeMeta.bestRank === 'number' && prevScopeMeta.bestRank > 0
    ...
    const updatedMonth = {
        ...
        challenges: {
            ...
            ranking: {
                ...
                milestonesGranted: Array.from(milestonesGranted),
                meta: stripUndefined({ ...meta, scopes }),
            },
        },
    };
...
function buildMonthlyChallengeViews(state, now = new Date()) {
    const mk = monthKeyFromLocalDate(now);
    const month = state.months[mk] ?? { monthKey: mk, challenges: {}, appliedRunIds: [], appliedEventIds: [] };
    return monthlyChallengesConfig_1.monthlyChallenges.map((def) => {
        ...
        const earnedXpThisStage = (starsEarned >= 1 ? xpForMilestone(def.id, safeStage, 1) : 0) +
        ...
        const nextMilestoneXp = starsEarned < 3
            ? xpForMilestone(def.id, safeStage, (starsEarned + 1))
            : undefined;
        let meta = baseMeta;
        if (def.id === 'ranking') {
            meta = {
                scopes: baseMeta?.scopes ?? {},
            };
        }
        return {
            id: def.id,
